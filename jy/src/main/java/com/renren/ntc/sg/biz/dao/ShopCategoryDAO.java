...
public interface ShopCategoryDAO {
    ...
    @SQL("select * from " + TABLE_NAME + " where shop_id =:1 and category_id = :2")
    public ShopCategory getByCategoryId(long shop_id, int category_id);

    @SQL("select "+ FIELDS +"  from " + TABLE_NAME + " where shop_id = :1 and name = :2")
    public ShopCategory getByName(long shop_id, String goodsCategory);
}
