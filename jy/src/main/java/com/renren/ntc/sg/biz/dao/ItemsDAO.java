...
public interface ItemsDAO {
    ...
    @SQL("select " + FIELDS + " from ##(:tableName)   where shop_id=:2 and id in (:3)")
    public List<Item> getItemsByIdsAnShopId(@SQLParam("tableName") String tableName, long shopId, List<Long> itemIds);

    @SQL("select  count(*) from ##(:tableName) where shop_id =:2 and category_id=:3 order by score desc")
    public int getItemsWithZeroCount(@SQLParam("tableName") String tableName, long shop_id, int category_id);
}
