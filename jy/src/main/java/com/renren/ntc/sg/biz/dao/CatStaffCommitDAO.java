...
public interface CatStaffCommitDAO {
    ...
    static final String FIELDS = "id,name ,phone,pwd ,shop_id,shop_info,shop_serveArea," +
            "shop_name,shop_owner_phone,shop_address,shop_tel,shop_print,shop_lat,shop_lng,create_time,update_time ,serialNo_num, success_num,shop_area";
    static final String INSERT_FIELDS = "name ,phone,pwd ,shop_serveArea,shop_name," +
            "shop_owner_phone,shop_address,shop_tel,shop_print,shop_lat,shop_lng,serialNo_num, success_num,shop_area";
    ...
    @SQL("insert into " + TABLE_NAME + " (" + INSERT_FIELDS + " ) values (:1.name ,:1.phone,:1.pwd ," +
            ...
            ":1.shop_print,:1.shop_lat,:1.shop_lng,:1.serialNo_num,:1.success_num,:1.shop_area) ")
     ...
     * @return
     */
    @SQL("select " + FIELDS + " from " + TABLE_NAME + " where  name like :1 or shop_name like :1  order by name asc limit :2 , :3")
    public List<CatStaffCommit> getCatStaffCommit(String key, int from, int offset);
    /**
     * @param key
     * @return
     */
    @SQL("select count(id) from " + TABLE_NAME + " where  name like :1 or phone like :1 or shop_name like :1 or shop_address like :1")
    public int getCatStaffCommitCount(String key);
    ...
    @SQL("update " + TABLE_NAME + " set ##(:key) = :2 where shop_id =:3")
    public int updateByShopId(@SQLParam("key") String key, String value, long id);
    ...
    public List<CatStaffCommit> getPhone(String phone);

    @SQL("select  shop_area from " + TABLE_NAME + " group by shop_area")
    public List<String> getAreas();

    @SQL("select shop_id from " + TABLE_NAME + " where  shop_area =:1 ")
    public List<Long> getShop_idsByarea(String shop_area);

    @SQL("select  shop_id from " + TABLE_NAME + " where phone =:1")
    public List<Long> getShop_idsByPhone(String shop_area);

    @SQL("select DISTINCT (shop_area) from " + TABLE_NAME + " where  shop_area like :1 ")
    public List<CatStaffCommit> getArea(String shop_area);
}
