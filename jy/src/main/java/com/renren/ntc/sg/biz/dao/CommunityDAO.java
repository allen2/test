...
public interface CommunityDAO {
    ...
    @SQL("select "+ FIELDS +" from " + TABLE_NAME + " where name like :1 order by id desc limit :2,:3")
    List<Community> getByQuery(String value, int from, int offset);

    @SQL("update " + TABLE_NAME + " set ##(:key) = :2 , update_time = now() where id =:3")
    int update(@SQLParam("key") String key, String value, long id);
