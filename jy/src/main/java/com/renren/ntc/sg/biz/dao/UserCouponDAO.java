...
public interface UserCouponDAO {
    ...
    static final String  FIELDS = "id,user_id,coupon_id,shop_id,pic_url,code,status, price ,name  ,ext,start_time,end_time,create_time,update_time,rule"  ;
    static final String  INSERT_FIELDS = "user_id,coupon_id,shop_id,pic_url,code ,status, price ,name,ext,start_time,end_time,rule"  ;
    ...
    @SQL("insert into " + TABLE_NAME + " ( " + INSERT_FIELDS + ") values (:1.user_id,:1.coupon_id,:1.shop_id,:1.pic_url,:1.code ,:1.status,:1.price ," +
            ":1.name ,:1.ext,:1.start_time,:1.end_time,:1.rule) ")
    ...
    public List<UserCoupon> geShopCoupons (long user_id, long shop_id ,int status ,int from,int offset ,java.util.Date now) ;


    @SQL("select " + FIELDS + " from " + TABLE_NAME +" where  user_id =:1 and shop_id = :2 order by status asc,end_time desc limit :3,:4 ")
    public List<UserCoupon> geShopAllCoupons (long user_id, long shop_id  ,int from,int offset) ;


    @SQL("select " + FIELDS + " from " + TABLE_NAME +" where  user_id =:1 and id = :2 and code=:3 and status = :4 and start_time <= :5  and  end_time >= :5 ")
    public UserCoupon getTicket(long user_id, long coupon_id, String coupon_code, int couponunused,java.util.Date now);
    ...
    @SQL("select " + FIELDS + " from " + TABLE_NAME +" where  id =:1 and user_id =:2")
    public UserCoupon getUser_CouponByCondition(   long couponId, long userId) ;
    
    @SQL("select max(coupon_id) from " + TABLE_NAME +" where  user_id =:1")
    public Integer getMaxUserCouponId(long userId) ;
    
    @SQL("select count(*) from " + TABLE_NAME +" where  user_id =:1")
    public Integer getMyCouponCount(long user_id);
}